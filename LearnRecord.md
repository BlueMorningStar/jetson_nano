...
3. 内存不够(SWAP)
4. 频繁申请释放(内存池)
5. 越运行，越卡，甚至死机(内存泄露，valgrind)
6. 多任务，多设备最优化(无锁化，环形缓存区)
7. 安全 易用 独立(映射)
8. 内核到应用空间 频繁拷贝(mmap)
...
1. 映射的最小单位:MMU中VA(虚拟空间)到PA(物理空间)映射的最小单位称为页(page) = 4K.
...
1. RTMP:Real Time Messaging Protocol 实时消息传输协议 -->直播，通话(浏览器支持好)
2. RTSP:Real Time Streaming Protocol 实时流传输协议 --> 安防监控，视频聊天
3. HLS: Http Live Streaming 基于Http的流媒体网络传输协议 -->点播(延时大)
4. RTP: Real-time Transport Protocol 实时传输协议
5. WebRTC:Web Real-Time Communication 网页即时通信(web+rtsp) --> web页面的视频会议，直播
发布端

1. ffmpeg: 跨平台音视频解决方案，用于录制、转换和流式传输 --> 传统 音视频解码领域，没有图像界面，提供三个命令ffmpeg, ffplay,ffprobe
   优:简单上手快 (简洁高效 适合做简单音视频应用 国内用很多)
   缺:AI, 视频分析等扩展差

2. gstreamer: 流媒体框架(跨平台 开源) --> 面向AI， 视频分析等复杂音视频功能
   优:高度模块化，扩展性强 (英伟达的DeepStream视频分析系统就是基于它)
